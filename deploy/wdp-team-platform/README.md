...
| **四个 AI Agent** | ①对话 Agent（成员日常）②归并 Agent（信号池分析给归并建议，n 对 n 分组逐组拍板）③审核 Agent（归类/查重/质量分析，**对照成员职责给出建议负责人**）④团队规则 Agent（对话共创团队 SOUL，一键应用并发布）；②③④决策记录为 few-shot 学习素材，越用越贴合团队尺度。 |
| **成员职责与智能分配** | 管理员在成员管理页定义每人职责域（负责的产品方向/模块）；审核 Agent 据此在入库时给出「建议分配给谁」，为主 Agent 的派活能力提供抓手。 |
