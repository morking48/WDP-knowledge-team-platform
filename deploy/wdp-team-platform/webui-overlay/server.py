...
def main() -> None:
    ...
    print("[progress] 5% - Config loaded", flush=True)
    ...
    print("[progress] 10% - File permissions OK", flush=True)
    ...
    try:
        ...
        print("[progress] 15% - Recovering sessions...", flush=True)
    ...
    print("[progress] 40% - Sessions recovered", flush=True)
    ...
    print("[progress] 50% - Verifying dependencies...", flush=True)
    ...
    print("[progress] 65% - Dependencies OK", flush=True)

    # Start the gateway session watcher for real-time SSE updates
    print("[progress] 70% - Starting gateway watcher...", flush=True)
    try:
    ...
    print("[progress] 85% - Gateway watcher started", flush=True)
    ...
    print('[progress] 100% - Ready!', flush=True)

    # ── WDP 团队工作台：主 Agent 定时任务调度器（仅多用户模式）──────────────
    try:
        import os as _os
        if _os.getenv('HERMES_WEBUI_MULTIUSER', '').strip() in ('1', 'true', 'yes'):
            from api import team_scheduler as _sched
            if _sched.start():
                print('  WDP team-scheduler started', flush=True)
    except Exception as _e:
        print(f'[!] team-scheduler start skipped: {_e}', flush=True)
