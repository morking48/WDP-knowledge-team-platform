...
def _platform_default_hermes_home() -> Path:
    ...
    TEAM DEPLOYMENT: HERMES_HOME env var (when set) takes highest priority so a
    self-contained team instance can pin its own home dir and avoid the
    Windows AppData vs ~/.hermes split that broke config loading. This makes the
    web-ui read the SAME home the agent uses, from import time onward.
    """
...
def get_available_models() -> dict:
    ...
    def _build_available_models_uncached() -> dict:
        ...
        def _build_configured_model_badges() -> dict[str, dict[str, str]]:
            ...
            # Favorite/常用 models: pin to the top of the picker WITHOUT joining
            # the fallback chain (so they never trigger silent auto-switch).
            # Config: top-level ``favorite_models: [{provider, model}, ...]``.
            favorite_cfg = cfg.get("favorite_models", [])
            if isinstance(favorite_cfg, list):
                for entry in favorite_cfg:
                    if not isinstance(entry, dict):
                        continue
                    provider = _resolve_provider_alias(entry.get("provider"))
                    model = str(entry.get("model") or "").strip()
