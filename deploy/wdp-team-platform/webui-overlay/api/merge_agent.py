...
- **完整度（价值字段检查）**：检查非必填但有价值的字段是否空缺或写着"待补充"（需求的 business_value/customer/target_release、设计的 designer/doc_url 等）。**能从内容推导的直接给出建议值**（在 proposal 的 suggested_fields 里），推导不了的列入 missing_fields 提示管理员。不要放任"(待补充)"入库。
- 查重：与现有知识库条目对比，主题重复的标记重复风险并指出疑似条目。
- **迭代识别（高频场景）**：提交内容若是**已有条目的进展更新/补充**（如早会说"XX 任务本周完成了 POC"，而 XX 已有需求/信号条目），**不要建议新建也不要驳回**——recommendation 填「合并更新」，merge_into 填目标条目 id，merge_note 用一句话提炼增量（如"2026-07-30：POC 完成，进入联调"）；若进展暗示状态变化（完成/暂停/上线），在 suggested_status 里给出目标条目的建议新状态。判别口诀：**同一件事的新进展→合并更新；真正的新事项→新建**。
