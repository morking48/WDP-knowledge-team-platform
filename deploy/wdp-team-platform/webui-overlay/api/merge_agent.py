...
CATEGORY_GUIDE = """## 类目判别标准（两个维度，按顺序判）

**维度一（先判）：生命周期——新事还是旧事的延续？**
- **新冒出、未定如何执行**（市场/销售一线反馈、会议中流出的新想法、刚发现的情况）→ **signals 信号**。哪怕描述写得很完整，只要是新线索就是信号。
- **有历史背景、有进度迭代痕迹**（"上次说的XX""XX有进展/完成了""继XX之后"）→ 这是**已有条目的更新**，不是新条目——找到对应条目走合并更新，而非新建。
- ⚠ 反例警示：**描述完整≠需求，简陋≠信号——看时间性，不看文笔**。

**维度二（后判，确认是新条目后）：内容形态**
- 事实/线索→signals ｜ 明确诉求/待办（做什么+为谁做，可验收）→requirements ｜ 方案/文档（怎么做）→designs ｜ 已拍板定论→decisions ｜ 客户项目立项建档→projects
- 混含多类按主体判，提示可拆分；拿不准倾向 signals（入口层可流转），说明拿不准的原因。"""
