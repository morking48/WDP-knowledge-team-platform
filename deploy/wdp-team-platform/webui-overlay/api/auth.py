...
def check_auth(handler, parsed) -> bool:
    ...
    if cookie_val and verify_session(cookie_val):
        # 多用户模式：验签后还要确认该 session 仍是用户的 active_token
        # （单点登录——后登录挤掉先登录；被停用/被挤下线的旧 session 判为失效）
        try:
            from api import users as _users
            if _users.multiuser_enabled():
                if not _users.session_username(cookie_val):
                    # session 签名有效，但已非任何用户的活跃 session（被挤掉/已登出）
                    _reject_unauthorized(handler, parsed)
                    return False
        except Exception:
            # 多用户校验异常不阻断主流程（回落为已认证），避免锁死
            pass
        return True
    # Not authorized
    _reject_unauthorized(handler, parsed)
