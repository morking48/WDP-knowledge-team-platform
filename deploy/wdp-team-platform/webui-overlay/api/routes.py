...
def _resolve_context_length_for_session_model(
    ...
    try:
        ...
        try:
            ...
            pass
        # Resolve the custom provider's base_url so the per-model context_length
        # override (custom_providers[].models.<model>.context_length) can match.
        # get_custom_provider_context_length() keys the override on base_url, so
        # passing "" here silently skips it and falls back to the 200K default
        # for models like claude-opus-4-8 that have a 1M window. The session
        # provider string is "custom:<name>"; strip the prefix to match name.
        _base_url_load = ""
        try:
            _prov_name = (provider or "").split(":", 1)[1] if ":" in (provider or "") else ""
            if _prov_name and isinstance(_cfg_custom_providers_load, list):
                for _cp in _cfg_custom_providers_load:
                    if isinstance(_cp, dict) and _cp.get("name") == _prov_name:
                        _base_url_load = str(_cp.get("base_url") or "")
                        break
        except Exception:
            _base_url_load = ""
        try:
...
def _collapse_local_session_lineage(sessions: list[dict]) -> list[dict]:
    """Collapse compression-lineage chains for non-messaging (local/WebUI) sessions.

    When a session is context-compressed, the pre-compression snapshot is kept
    as a parent session and a new child session continues the conversation. Both
    share the same title and show up as separate sidebar rows. Messaging
    sessions are de-duplicated elsewhere by source identity; local sessions had
    no such collapsing, so the parent + child both appeared.

    Rule: hide a local session when another session in the same list points to
    it via ``parent_session_id`` (i.e. it is a superseded link in a lineage
    chain). Only collapse when BOTH parent and child are present in the list and
    the row being hidden is itself a local (non-messaging) session, so a parent
    that exists on its own is never made to vanish.
    """
    # session_ids that are referenced as a parent by some other row in the list
    referenced_parents: set[str] = set()
    for session in sessions:
        if not isinstance(session, dict):
            continue
        parent = _safe_first(session.get("parent_session_id"))
        if parent:
            referenced_parents.add(str(parent))
    ...
    for session in sessions:
        if isinstance(session, dict):
            # Only collapse local (non-messaging) sessions; messaging rows are
            # handled by source-identity de-duplication.
            if _messaging_source_key(session) is None:
                sid = _safe_first(session.get("session_id"))
                if sid and str(sid) in referenced_parents:
                    continue  # superseded parent in a lineage chain — hide it
...
def _keep_latest_messaging_session_per_source(
    ...
    *,
    ...
    sessions = _collapse_local_session_lineage(sessions)
...
# ── 多用户登录页（WDP 团队平台：绿白 + 液体玻璃 + 淡网格）────────────────────
_LOGIN_PAGE_HTML_MULTIUSER = """<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>{{BOT_NAME}} — 团队工作台登录</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
:root{
  --brand:#16a34a;--brand-strong:#15803d;--brand-soft:rgba(22,163,74,.12);
  --ink:#0f1f16;--ink-2:#4b5d52;--line:rgba(22,163,74,.18);
}
...
body{
  ...
  background:
    ...
    linear-gradient(160deg,#f4faf6 0%,#eef7f0 50%,#e9f4ec 100%);
  background-size:28px 28px,28px 28px,100% 100%;
  min-height:100vh;padding:20px;
}
.card{
  width:100%;max-width:360px;padding:38px 34px 30px;text-align:center;
  ...
  box-shadow:
    0 8px 40px rgba(22,163,74,.10),
    inset 0 1px 0 rgba(255,255,255,.8),
    inset 0 -1px 0 rgba(22,163,74,.06);
}
.logo{
  ...
  box-shadow:0 6px 20px rgba(22,163,74,.32),inset 0 1px 0 rgba(255,255,255,.35);
}
h1{font-size:20px;font-weight:700;letter-spacing:.02em}
.sub{font-size:13px;color:var(--ink-2);margin:6px 0 26px}
.field{position:relative;margin-bottom:14px;text-align:left}
.field input{
  width:100%;padding:12px 15px;border-radius:14px;font-size:14px;color:var(--ink);
  background:rgba(255,255,255,.7);
  border:1px solid var(--line);outline:none;
  transition:border-color .15s,box-shadow .15s,background .15s;
}
.field input::placeholder{color:#9ab3a3}
.field input:focus{
  border-color:var(--brand);background:#fff;
  box-shadow:0 0 0 4px var(--brand-soft);
}
button[type=submit]{
  width:100%;padding:12px;margin-top:6px;border:none;border-radius:14px;cursor:pointer;
  font-size:15px;font-weight:600;color:#fff;letter-spacing:.02em;
  background:linear-gradient(145deg,#22c55e,#16a34a);
  box-shadow:0 6px 18px rgba(22,163,74,.3),inset 0 1px 0 rgba(255,255,255,.3);
  transition:transform .12s,box-shadow .15s,filter .15s;
}
button[type=submit]:hover{filter:brightness(1.05);box-shadow:0 8px 24px rgba(22,163,74,.38)}
button[type=submit]:active{transform:scale(.98)}
button[type=submit]:disabled{opacity:.6;cursor:not-allowed}
.err{
  color:#dc2626;font-size:13px;margin-top:14px;display:none;
...
<div class="card">
  ...
  <form id="login-form" data-invalid-pw="账号或密码错误" data-conn-failed="连接失败，请稍后再试">
    <div class="field"><input type="text" id="username" placeholder="账号" autocomplete="username" autofocus></div>
    <div class="field"><input type="password" id="pw" placeholder="密码" autocomplete="current-password"></div>
    <button type="submit">登 录</button>
  </form>
  <div class="err" id="err"></div>
  <div class="foot">使用团队管理员分配的账号登录</div>
</div>
<script src="static/login-multiuser.js?v={{WEBUI_VERSION}}"></script>
</body></html>"""
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path in ("/", "/index.html") or parsed.path.startswith("/session/"):
        try:
            ...
            # WDP 团队工作台：多用户模式下用客制化前端 workbench.html 替换官方 index
            _page_path = _INDEX_HTML_PATH
            _is_wdp_workbench = False
            try:
                from api import users as _wdp_users
                if _wdp_users.multiuser_enabled():
                    _wb_path = _INDEX_HTML_PATH.parent / "workbench.html"
                    if _wb_path.exists():
                        _page_path = _wb_path
                        _is_wdp_workbench = True
            except Exception:
                pass

            # 客制化工作台：版本号基于 static/wb*.js/wb.css 的最大 mtime，
            # 保证每次改前端代码 URL 自动变、浏览器自动加载新版（根治缓存）
            if _is_wdp_workbench:
                try:
                    _sdir = _page_path.parent
                    _mt = 0.0
                    for _fn in ("wb-modal.js", "wb.js", "wb2.js", "wb3.js", "wb4.js", "wb.css", "workbench.html"):
                        _fp = _sdir / _fn
                        if _fp.exists():
                            _mt = max(_mt, _fp.stat().st_mtime)
                    if _mt:
                        version_token = quote(str(int(_mt)), safe="")
                except Exception:
                    pass
            ...
            # 客制化工作台自带完整 HTML，不注入官方扩展标签（避免加载官方 JS 冲突）
            if _page_path != _INDEX_HTML_PATH:
...
# ── Providers (GET) ──
    ...
    if parsed.path == "/api/gateway/status":
        return j(handler, _gateway_status())
    ...
    if parsed.path == "/api/git-info":
        ...
        try:
            ...
            # Defensive: never let an unexpected git failure 500 this endpoint.
            # The git badge is non-essential UI (the frontend loads it
            # fire-and-forget), so degrade gracefully to "no git info" instead
            # of surfacing a 500 that clutters the console on every session load.
            import logging
            logging.getLogger("webui").warning(
                "git-info: unexpected git_status failure for %s", s.workspace,
                exc_info=True,
            )
    ...
    # ── 多用户：当前登录用户 / 用户列表（GET，WDP 团队平台）────────────
    if parsed.path == "/api/auth/me":
        from api import users as _users
        if not _users.multiuser_enabled():
            return j(handler, {"multiuser": False})
        u = _users.current_request_user(handler)
        if not u:
            return j(handler, {"error": "未登录"}, status=401)
        return j(handler, {"multiuser": True, "user": u})

    if parsed.path == "/api/admin/users":
        from api import users as _users
        if not _users.multiuser_enabled():
            return j(handler, {"error": "多用户模式未启用"}, status=400)
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        from urllib.parse import parse_qs as _pq2
        _stats = (_pq2(parsed.query).get("stats") or ["0"])[0] == "1"
        return j(handler, {"users": _users.list_users(with_stats=_stats)})
    ...
    if parsed.path == "/api/admin/tasks":
        ...
        return j(handler, _tt.list_tasks())

    # ── 团队 Agent 配置（admin，P1）────────────────────────────────────
    if parsed.path == "/api/admin/team-agent":
        from api import users as _users
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
    ...
    if parsed.path == "/api/admin/output-board":
        ...
        return j(handler, _ops.team_output_board())

    # ── 归并 Agent 分析 + 规则（admin，R10）────────────────────────────
    if parsed.path == "/api/admin/merge/analyze":
        from api import users as _users
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        from api import merge_agent as _ma
        res = _ma.analyze_merge()
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, res)
    if parsed.path == "/api/admin/merge/rule":
        from api import users as _users
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        from api import merge_agent as _ma
    ...
    if parsed.path == "/api/admin/agent-log/stats":
        ...
        return j(handler, {"merge": _al.history_stats("merge"), "review": _al.history_stats("review")})

    # ── WDP 团队工作台 R1：知识库只读接口 ─────────────────────────────
    if parsed.path.startswith("/api/knowledge/"):
        from api import knowledge as _kb
        from api import users as _users
        # 登录即可读（成员只读；写操作走 R8 admin 接口）
        if _users.multiuser_enabled():
            u = _users.current_request_user(handler)
            if not u:
                return j(handler, {"error": "未登录"}, status=401)
        ...
        if sub == "item":
            ...
            if isinstance(res, tuple):
                body, code = res
                return j(handler, body, status=code)
            return j(handler, res)
        if sub == "traces":
            from api import knowledge_ops as _ops
            from urllib.parse import parse_qs as _pq
            ...
            it = (q.get("type") or [""])[0]
            iid = (q.get("id") or [""])[0]
            return j(handler, _ops.get_traces(it, iid))
        if sub == "req-decisions":
            from api import knowledge_ops as _ops
            from urllib.parse import parse_qs as _pq
            iid = (_pq(parsed.query).get("id") or [""])[0]
            return j(handler, _ops.get_req_decisions(iid))
        if sub == "library":
            from api import knowledge_ops as _ops
    ...
    if parsed.path.startswith("/api/review/"):
        ...
        if not u:
            return j(handler, {"error": "未登录"}, status=401)
        sub = parsed.path[len("/api/review/"):]
        if sub == "list":
            if not _users.is_request_admin(handler):
                return j(handler, {"error": "需要管理员权限"}, status=403)
            return j(handler, _rv.handle_review_list(handler, parsed))
        if sub == "item":
            # item 允许提交人本人或 admin 查看（简化：admin）
            if not _users.is_request_admin(handler):
                return j(handler, {"error": "需要管理员权限"}, status=403)
            res = _rv.handle_review_item(handler, parsed)
            if isinstance(res, tuple):
                body, code = res
                return j(handler, body, status=code)
            return j(handler, res)
        return j(handler, {"error": "未知 review 子路径"}, status=404)

    # ── WDP 团队工作台 R4：个人中心（GET 部分）─────────────────────────
    if parsed.path.startswith("/api/me/"):
        from api import me as _me
        sub = parsed.path[len("/api/me/"):]
        if sub == "agent":
            res = _me.handle_me_agent(handler, parsed)
        elif sub == "team-rules":
            # #8：成员只读真实团队规则（发布母本），不需要 admin
            from api import team_agent as _ta
            ...
            res = _me.handle_me_logs(handler, parsed)
        elif sub == "logs/download":
            # 日志下载（raw 文本）
            from urllib.parse import parse_qs as _pq
            import os as _os
            _u, _home = _me._current(handler)
            if not _u or not _home:
                return j(handler, {"error": "未登录"}, status=401)
            _fn = (_pq(parsed.query).get("file") or [""])[0]
            _safe = _os.path.basename(_fn)
            _lp = _home / "logs" / _safe
            if not _safe or not _lp.is_file():
                return j(handler, {"error": "日志不存在"}, status=404)
            try:
                _data = _lp.read_bytes()
            except Exception as _e:
                return j(handler, {"error": str(_e)}, status=500)
            handler.send_response(200)
            handler.send_header("Content-Type", "text/plain; charset=utf-8")
            handler.send_header("Content-Disposition", f'attachment; filename="{_safe}"')
            handler.send_header("Content-Length", str(len(_data)))
            handler.end_headers()
            handler.wfile.write(_data)
            return True
        elif sub == "channels":
            ...
            if _users.multiuser_enabled() and not _users.current_request_user(handler):
                return j(handler, {"error": "未登录"}, status=401)
            res = _ch.list_devices()
        elif sub == "environment":
            from api import channels as _ch
            from api import users as _users
            if _users.multiuser_enabled() and not _users.current_request_user(handler):
                return j(handler, {"error": "未登录"}, status=401)
            res = _ch.get_environment()
        elif sub == "notifications":
            from api import knowledge_ops as _ops
            from api import users as _users
            if _users.multiuser_enabled() and not _users.current_request_user(handler):
                return j(handler, {"error": "未登录"}, status=401)
            res = _ops.list_notifications(handler)
        elif sub == "members":
            # ④ @通知：登录成员可拿轻量成员名单（仅用户名/角色，供@提及）
            from api import users as _users
            if _users.multiuser_enabled() and not _users.current_request_user(handler):
                return j(handler, {"error": "未登录"}, status=401)
            res = {"members": [{"username": x.get("username"), "role": x.get("role")}
                               for x in _users.list_users() if x.get("active", True) is not False]}
        else:
            return j(handler, {"error": "未知 me 子路径"}, status=404)
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, res)
...
def handle_post(handler, parsed) -> bool:
    ...
    # ── WDP 团队工作台 R6：上传到个人 workspace ────────────────────────
    if parsed.path == "/api/me/upload":
        from api import workspace_upload as _wu
        return _wu.handle_workspace_upload(handler)
    ...
    # ── 多用户管理（WDP 团队平台，admin 专属，写操作走 POST）──────────
    if parsed.path == "/api/admin/users/create":
        ...
        try:
            rec = _users.create_user(username, password, role=role)
        except ValueError as e:
        ...
        return j(handler, {"ok": True, "user": rec, "profile_created": profile_created})

    if parsed.path == "/api/admin/users/reset_password":
        from api import users as _users
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        username = (body.get("username") or "").strip()
        ...
        try:
            ok = _users.reset_password(username, new_password)
        except ValueError as e:
            return bad(handler, str(e), 400)
        return j(handler, {"ok": ok})

    if parsed.path == "/api/admin/users/set_active":
        from api import users as _users
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        username = (body.get("username") or "").strip()
        active = bool(body.get("active", True))
        ok = _users.set_user_active(username, active)
        return j(handler, {"ok": ok})

    if parsed.path == "/api/admin/users/kick":
        from api import users as _users
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        username = (body.get("username") or "").strip()
    ...
    if parsed.path == "/api/review/submit":
        from api import review as _rv
        res = _rv.handle_review_submit(handler, body)
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, res)
    ...
    if parsed.path == "/api/review/approve":
        ...
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        res = _rv.handle_review_approve(handler, body)
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, res)
    ...
    if parsed.path == "/api/review/reject":
        ...
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        res = _rv.handle_review_reject(handler, body)
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, res)
    ...
    # ── WDP 团队工作台 R4：个人中心（POST 部分）────────────────────────
    if parsed.path == "/api/me/soul":
        from api import me as _me
        res = _me.handle_soul_write(handler, body)
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, res)

    if parsed.path == "/api/me/memory":
        from api import me as _me
        res = _me.handle_memory_write(handler, body)
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, res)
    ...
    if parsed.path.startswith("/api/me/channels/") or parsed.path.startswith("/api/me/devices/") or parsed.path.startswith("/api/me/workspaces/"):
        ...
        p = parsed.path
        try:
            if p == "/api/me/channels/save":
                res = _ch.save_channel(body)
            elif p == "/api/me/channels/delete":
                res = _ch.delete_channel(body.get("id"))
            elif p == "/api/me/channels/test":
                ...
                res = _ch.remove_device(body.get("id"))
            elif p == "/api/me/workspaces/save":
                res = _ch.save_workspace_entry(body)
            elif p == "/api/me/workspaces/activate":
                res = _ch.set_active_workspace(body.get("id") or "")
            elif p == "/api/me/workspaces/remove":
                res = _ch.remove_workspace_entry(body.get("id"))
            else:
                return j(handler, {"error": "未知子路径"}, status=404)
        except Exception as e:
            return j(handler, {"error": str(e)}, status=500)
    ...
    # ── WDP 团队工作台 R8：知识库管理操作（admin）──────────────────────
    if parsed.path == "/api/admin/knowledge/update":
        from api import users as _users
        from api import knowledge_admin as _kadm
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        return _kadm.handle_admin_knowledge_update(handler, body)

    # ── 团队 Agent 配置操作（admin，P1）────────────────────────────────
    if parsed.path.startswith("/api/admin/team-agent/") or parsed.path.startswith("/api/admin/agent-dialog/"):
        from api import users as _users
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        from api import team_agent as _ta
        p = parsed.path
        try:
            if p == "/api/admin/team-agent/soul":
            ...
            elif p == "/api/admin/agent-dialog/send":
                from api import agent_dialog as _ad
                res = _ad.send_dialog(body.get("dialog_id") or "", body.get("message") or "")
            elif p == "/api/admin/agent-dialog/close":
                from api import agent_dialog as _ad
                res = _ad.close_dialog(body.get("dialog_id") or "")
            else:
    ...
    # ── 主 Agent 定时任务操作（admin）────────────────────────────────
    if parsed.path.startswith("/api/admin/tasks/"):
        from api import users as _users
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        from api import team_tasks as _tt
        p = parsed.path
        try:
            if p == "/api/admin/tasks/update":
                res = _tt.update_task(body.get("id") or "", body.get("updates") or {})
            elif p == "/api/admin/tasks/create":
                res = _tt.create_custom_task(body.get("name") or "", body.get("schedule") or "", body.get("prompt") or "")
            elif p == "/api/admin/tasks/delete":
                res = _tt.delete_task(body.get("id") or "")
            elif p == "/api/admin/tasks/run":
                res = _tt.run_task(body.get("id") or "", triggered_by="manual")
            else:
                return j(handler, {"error": "未知任务操作"}, status=404)
        except Exception as e:
            return j(handler, {"error": str(e)}, status=500)
        if isinstance(res, tuple):
            b, code = res
    ...
    # ── 工作台高级操作（对齐原型：归并/沉淀/新建/通知，均 admin）────────
    if parsed.path.startswith("/api/knowledge/") and parsed.path in (
        ...
        if not _users.is_request_admin(handler):
            return j(handler, {"error": "需要管理员权限"}, status=403)
        u = _users.current_request_user(handler)
        admin = (u or {}).get("username", "admin")
        p = parsed.path
        try:
            if p == "/api/knowledge/merge":
                res = _ops.merge_signals(body.get("ids") or [], body.get("title") or "", admin,
                                         new_body=body.get("body") or "", new_urgency=body.get("urgency") or "")
            elif p == "/api/knowledge/to-requirement":
                res = _ops.signal_to_requirement(
                    body.get("signal_id") or "", admin,
                    body.get("priority") or "P2", body.get("owner") or "")
            elif p == "/api/knowledge/new-design":
                res = _ops.new_design(body.get("title") or "", body.get("requirement_id") or "", admin)
            elif p == "/api/knowledge/notify":
                res = _ops.notify_member(body.get("username") or "", body.get("message") or "", admin)
            elif p == "/api/knowledge/delete-item":
    ...
    # ④ @通知：成员间快速通知（任何登录成员可发，非admin-only）
    if parsed.path == "/api/me/mention":
        from api import users as _users
        from api import knowledge_ops as _ops
        u = _users.current_request_user(handler)
        if not u:
        ...
        if not target or not msg:
            return j(handler, {"error": "缺 username/message"}, status=400)
        valid = {x.get("username") for x in _users.list_users() if x.get("active", True) is not False}
        if target not in valid:
            return j(handler, {"error": f"成员 {target} 不存在"}, status=404)
        res = _ops.notify_member(target, f"💬 @{u.get('username')}：{msg}", u.get("username"))
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, res)
    ...
    # 工作库重索引（个人）
    if parsed.path == "/api/me/workspace/reindex":
        from api import users as _users
        from api import me as _me
        if _users.multiuser_enabled() and not _users.current_request_user(handler):
            return j(handler, {"error": "未登录"}, status=401)
        # 复用 handle_me_workspace 重新扫描（本就是实时扫描，返回最新即可）
        res = _me.handle_me_workspace(handler, parsed)
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, {"ok": True, "workspace": res})

    # 通知：标记全部已读（个人）
    if parsed.path == "/api/me/notifications/read":
        from api import users as _users
        from api import knowledge_ops as _ops
        if _users.multiuser_enabled() and not _users.current_request_user(handler):
            return j(handler, {"error": "未登录"}, status=401)
        res = _ops.mark_notifications_read(handler)
        if isinstance(res, tuple):
            b, code = res
            return j(handler, b, status=code)
        return j(handler, res)
    ...
    if parsed.path == "/api/auth/login":
        ...
        if _users.multiuser_enabled():
            ...
            if not _check_login_rate(client_ip):
                return j(handler, {"error": "尝试次数过多，请稍后再试。"}, status=429)
            username = (body.get("username") or "").strip()
            password = body.get("password", "")
            u = _users.verify_user_credentials(username, password)
            if not u:
                _record_login_attempt(client_ip)
                return bad(handler, "账号或密码错误", 401)
            _clear_login_attempts(client_ip)
            # 登录（单点登录：新 token 覆盖 active_token，挤掉旧登录）
            result = _users.login_user(username)
            if not result:
                return bad(handler, "账号已被停用", 403)
            cookie_val, profile_name = result
            ...
            # 登录即绑定 profile（之后每个请求按此 profile 隔离）
...
def _serve_static(handler, parsed):
    ...
    # WDP 团队工作台的客制化前端文件用 no-cache（带 ETag 校验）：
    # 我们用 ?v=mtime 而非文件名指纹，immutable 会让浏览器强缓存旧版导致改动不生效。
    # no-cache 让浏览器每次带 If-None-Match 校验，未变返回 304（高效），变了自动下载新版。
    _wdp_files = ('wb.js', 'wb2.js', 'wb3.js', 'wb4.js', 'wb5.js', 'wb-modal.js', 'wb.css', 'workbench.html')
    _is_wdp_custom = any(rel.endswith(f) for f in _wdp_files)
    if _is_wdp_custom:
...
def _gateway_status() -> dict:
    """Return Gateway service status for the status bar."""
    ...
    try:
        ...
        for line in out.stdout.strip().split('\n'):
            if line.strip():
                ...
                pids.append(parts[1].strip() if len(parts) > 1 else '')
        # Check gateway log for recent WeCom connection
        log_path = Path(os.path.expanduser("~/.hermes/logs/gateway.log"))
        if log_path.exists():
            lines = log_path.read_text(encoding="utf-8").splitlines()
            for line in reversed(lines):
                if "Connected to wss://openws.work.weixin.qq.com" in line:
                    result["wecom_connected"] = True
                    # Compute age since this connection. gateway.log timestamps
                    # are LOCAL time (not UTC), so parse them as naive local and
                    # subtract from naive local now(). Previously this tagged the
                    # timestamp as UTC and subtracted UTC now(), producing a
                    # bogus negative age (off by the local UTC offset, e.g. -8h).
                    try:
                        ts_str = line.split(" INFO")[0].strip()
                        from datetime import datetime
                        ts = datetime.strptime(ts_str, "%Y-%m-%d %H:%M:%S,%f")
                        age_s = (datetime.now() - ts).total_seconds()
                        result["wecom_last_connected_ago_s"] = int(age_s)
                    except Exception:
                        pass
                    break

            # Telegram status: scan for the most recent telegram lifecycle event
            # and decide connected vs down. Success → "Connected to Telegram";
            # down → "Connect attempt N/8 failed" or "Disconnected from Telegram".
            # Whichever appears LAST (scanning from the end) wins. Telegram in CN
            # needs a working proxy, so this often shows disconnected when the
            # proxy is down — that's accurate, not a bug.
            result["telegram_connected"] = False
            result["telegram_configured"] = False
            for line in reversed(lines):
                low = line
                is_up = "Connected to Telegram" in low
                is_down = ("failed: httpx" in low and "Telegram" in low) or \
                          "Disconnected from Telegram" in low
                if is_up or is_down:
                    result["telegram_configured"] = True
                    result["telegram_connected"] = is_up
                    try:
                        ts_str = line.split(" INFO")[0].split(" WARNING")[0].split(" ERROR")[0].strip()
                        from datetime import datetime
                        ts = datetime.strptime(ts_str, "%Y-%m-%d %H:%M:%S,%f")
                        result["telegram_last_event_ago_s"] = int((datetime.now() - ts).total_seconds())
                    except Exception:
                        pass
                    break
    except Exception:
    ...
    result["gateway_running"] = result["wecom_connected"]
    result["configured"] = True
    result["running"] = result["gateway_running"]
    return result
...
def _handle_media(handler, parsed):
    ...
    def _path_within_root(_target, _root):
        # os.path.commonpath raises ValueError on Windows when paths live on
        # different drives (e.g. C:\ hermes home vs F:\ workspace). Treat that
        # as "not within" instead of letting it crash the whole /api/media
        # handler, which would 500 and force every HTML/PDF/CSV preview to fall
        # back to a download link.
        try:
            return _os.path.commonpath([str(_target), str(_root)]) == str(_root)
        except ValueError:
            return False
    within_allowed = any(
        _path_within_root(target, root)
